...
7. Document the initial state.
   Add the first entry to `docs/ctx-orientation.md` with the repo purpose, structure, and current working boundaries.
...
- This canonical source repo stores its own state log under `docs/memory-ctx/`, but the default project setup contract for downstream repos stays `docs/ctx-orientation.md`.
