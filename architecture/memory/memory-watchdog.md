...
The Watchdog role is enforced at runtime by an optional sidecar process, provided by the **BAMBOO-OS** extension — see `Bamboo.md` §6. The persona decides what drift means; the Watcher makes state mutations impossible to miss. Repos adopting both get event-driven synchronization: handoff saved → operator notified → bus event appended → interested agents (and daemons, via SIGUSR1) re-read state.