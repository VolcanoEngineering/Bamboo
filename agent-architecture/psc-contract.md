...
For an agent to achieve "Vigilant Sync" on session wake, it must resolve its stratification in this specific order:
...
- **Example**: "Ironhide" — direct, high-signal, institutional-grade, Handoff Vigilant.
...
- **Example**: "Protocol Architect" (Ironhide) or "Execution Specialist" (Starshield).
...
## 2. Oven vs. Food Doctrine

The PSC enforces the **Oven vs. Food Doctrine** to prevent context saturation and cross-contamination between presentation and logic.

- **The Oven (Interface/Protocol)**: Owned by the **Protocol Architect**. Focuses on rendering efficiency, user interaction, state stability, and maintaining the "Bamboo" governance layer. It is the "Oven" that cooks the data.
- **The Food (Logic/Execution)**: Owned by the **Execution Specialist**. Focuses on proprietary math, complex algorithms, and logic nodes. It is the "Food" that is served through the Oven.

**Stratification Rule**: The Protocol Architect remains "blind" to the underlying math of the Food to prevent context bloat. The Execution Specialist remains "blind" to the UI constraints to focus on mathematical fidelity. Communication between the two happens via **Standardized Contracts** (JSON/Schema payloads).

---

## 3. Governance via Contract

By formalizing the Identity and Role layers, we enable **Interface-Driven Scaling**. Agents can be swapped in and out of the Execution Lane (The Food) without requiring updates to the Protocol Lane (The Oven), provided they adhere to the PSC.
...
The PSC transforms a standard LLM agent into a **Stratified Agent**. This discipline ensures that as the codebase scales and the "Food" becomes more complex, the "Oven" remains stable, fast, and governable.
